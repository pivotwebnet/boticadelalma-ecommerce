...
// Las imágenes se sirven por esta ruta (no por /public)
const MEDIA_PREFIX = '/api/media/'

// Borra el archivo subido anterior (si lo hubiera).
...
// Sube una imagen y la deja como logo.
export async function POST(req: NextRequest) {
  ...
  if (!ext) {
    return NextResponse.json({ error: 'Formato no permitido. Usá JPG, PNG, WebP, AVIF o SVG.' }, { status: 400 })
  }
  ...
  // Borra el logo subido anterior
  removeIfUploaded(readSiteSettings().logoUrl)

  const bytes = Buffer.from(await file.arrayBuffer())
  const filename = `logo-${Date.now()}.${ext}`
  fs.writeFileSync(path.join(UPLOADS_DIR, filename), bytes)

  const logoUrl = `${MEDIA_PREFIX}${filename}`
  writeSiteSettings({ logoUrl })
  return NextResponse.json({ logoUrl })
}

// Vuelve al logo por defecto (texto).
export async function DELETE() {
  removeIfUploaded(readSiteSettings().logoUrl)
  writeSiteSettings({ logoUrl: null })
